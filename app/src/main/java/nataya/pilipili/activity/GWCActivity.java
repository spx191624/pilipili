...
import nataya.pilipili.utils.PayKeys;
import nataya.pilipili.utils.PayResult;
import nataya.pilipili.utils.SignUtils;
            ...
            public void onClick(View v) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    if (list.get(i).getIschecked()){
                        MyApplication.getInstances().getDaoSession().getGoodBeanDao().delete(list.get(i));
                    }
                }
                initData();
                setPrice();
        ...
        switch (view.getId()) {
            ...
            case R.id.bt_goodinfo_confim:
                ...
                Runnable payRunnable = new Runnable() {
                    ...
                    public void run() {
                        // 构造PayTask 对象
                        PayTask alipay = new PayTask(GWCActivity.this);
                        // 调用支付接口，获取支付结果
            ...
            switch (msg.what) {
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        Toast.makeText(GWCActivity.this, "支付成功",
                                Toast.LENGTH_SHORT).show();
                    } else {
                        // 判断resultStatus 为非“9000”则代表可能支付失败
                        // “8000”代表支付结果因为支付渠道原因或者系统原因还在等待支付结果确认，最终交易是否成功以服务端异步通知为准（小概率状态）
                        if (TextUtils.equals(resultStatus, "8000")) {
                            Toast.makeText(GWCActivity.this, "支付结果确认中",
                                    Toast.LENGTH_SHORT).show();

                        } else {
                            // 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
                            Toast.makeText(GWCActivity.this, "支付失败",
                                    Toast.LENGTH_SHORT).show();
                ...
                case SDK_CHECK_FLAG: {
                    Toast.makeText(GWCActivity.this, "检查结果为：" + msg.obj,
                            Toast.LENGTH_SHORT).show();
                    break;
                }
                default:
